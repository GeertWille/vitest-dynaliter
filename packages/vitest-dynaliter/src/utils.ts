...
const detectTimers = (): boolean => {
  const usingVitestAndTimers =
    typeof vi !== 'undefined' && typeof globalObj.setTimeout !== 'undefined'

  let usingVitestFakeTimers = false
  if (
    usingVitestAndTimers &&
    typeof globalObj.setTimeout.clock !== 'undefined' &&
    typeof vi.getRealSystemTime !== 'undefined'
  ) {
    try {
      vi.getRealSystemTime()
      usingVitestFakeTimers = true
    } catch {
      // not using Jest's modern fake timers
    }
  }

  return usingVitestFakeTimers
}

// stolen from https://github.com/testing-library/dom-testing-library/blob/master/src/helpers.js
export const runWithRealTimers = <T, R>(callback: () => T | Promise<R>): T | Promise<R> => {
  const usingVitestFakeTimers = detectTimers()

  if (usingVitestFakeTimers) {
    vi.useRealTimers()
  }
